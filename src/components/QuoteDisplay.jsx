...
const QuoteDisplay = ({ quote, isFavorite, onToggleFavorite }) => {
  ...
  return (
    ...
    >
      ...
      {/* Main Card */}
      <div className="relative bg-white/20 dark:bg-gray-900/30 backdrop-blur-3xl rounded-3xl p-6 md:p-8 lg:p-12 shadow-2xl border border-white/30 dark:border-gray-700/30 hover:bg-white/25 dark:hover:bg-gray-900/40 transition-all duration-500">
        ...
        >
          <div className="flex items-center space-x-3">
            <div className="relative">
              <Quote className="w-8 h-8 md:w-10 md:h-10 text-blue-400 dark:text-blue-300" />
              <div className="absolute -inset-2 bg-blue-500/20 rounded-full blur-md animate-pulse"></div>
            </div>
            <span className="text-white/80 dark:text-gray-300/80 font-medium text-lg md:text-xl hidden sm:block">
              Ilhomli Iqtibos
            </span>
          </div>
          ...
          <motion.button
            ...
            className="group p-3 md:p-4 rounded-2xl bg-white/10 dark:bg-gray-800/20 backdrop-blur-xl border border-white/20 dark:border-gray-700/20 hover:bg-white/20 dark:hover:bg-gray-700/30 transition-all duration-300"
        ...
        >
          ...
          {settings.showAuthor && (
            ...
            >
              <div className="h-px bg-gradient-to-r from-transparent via-white/50 dark:via-gray-300/50 to-transparent w-8 md:w-16"></div>
              <div className="px-4 py-2 md:px-8 md:py-4 bg-white/15 dark:bg-gray-800/25 rounded-full backdrop-blur-sm border border-white/25 dark:border-gray-700/25">
                <p className="text-white/90 dark:text-gray-300/90 text-lg md:text-xl lg:text-2xl font-medium drop-shadow-lg">
                  {quote.author}
                </p>
              </div>
              <div className="h-px bg-gradient-to-r from-transparent via-white/50 dark:via-gray-300/50 to-transparent w-8 md:w-16"></div>
        ...
        <motion.div
          ...
          className="flex items-center justify-center space-x-1 md:space-x-2 flex-wrap gap-1 md:gap-2"
        >
          <Tag className="w-4 h-4 md:w-5 md:h-5 text-white/60" />
          {quote.tags?.map((tag, index) => (
            <span
              key={index}
              className="px-3 py-1 md:px-4 md:py-2 bg-white/10 dark:bg-gray-800/20 rounded-full text-white/70 dark:text-gray-300/70 text-sm md:text-base backdrop-blur-sm border border-white/20 dark:border-gray-700/20"
            >
              {tag}
            </span>
          ))}
